document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add Home and Empty Wishlist buttons
    const emptyWishlistButton = document.getElementById('empty-btn');
    emptyWishlistButton.addEventListener('click', emptyWishlist);
    ...
    wishlist.forEach(imagePath => {
        ...
        productElement.innerHTML = `
            ...
            <h3>${name}</h3>
            <p>Price: $${price}</p>
            <button class="btn remove-from-wishlist" data-image="${imagePath}">Remove</button>
        `;
...
function addEventListenersToRemoveButtons() {
    document.querySelectorAll('.remove-from-wishlist').forEach(button => {
        button.addEventListener('click', function() {
            removeFromWishlist(this.dataset.image);
        });
    });
}
...
function removeFromWishlist(image) {
    ...
    wishlist = wishlist.filter(item => item !== image);
    localStorage.setItem('wishlist', JSON.stringify(wishlist));
    window.location.reload(); // Refresh the page to update the wishlist display
}
